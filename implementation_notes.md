...
The formula's Δθ is a 1D quantity — it doesn't know about left-cuts vs right-cuts. The mapping from the formula's Δθ sign to SVG angle direction depends on which side of the OB→pocket line the cue ball sits on: when `cross(OB→pocket, OB→CB) ≥ 0`, positive Δθ maps to `dirAngle + Δθ` in SVG; when the cross product is negative, the mapping flips and the cone edges must swap. An initial implementation omitted this side-dependence, which caused the wider edge of the cone to appear on the wrong side for cuts where the cue ball is on the "far" side of the OB→pocket line.
